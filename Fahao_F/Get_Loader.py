...
class Get_Loader(object):
    ...
    def cifar_noniid(self):
        ...
        #     print(labels[idx])
        #     time.sleep(1)
        labels_list = {[] for i in range(0,10)}
        # print(labels_list[0])
        # labels_list = []
        for i in idxs:
            print(i, labels[i],labels_list)
            np.insert(labels_list[labels[i]],1,i)
...
def load_databatch(data_folder, idx, img_size=64):
    ...
    x = np.dstack((x[:, :int(img_size2/2)], x[:, int(img_size2/2):int(2*img_size2/2)], x[:, int(2*img_size2/2):]))
    print("sb")
