...
class Get_Loader(object):
    ...
    def cifar_noniid(self):
        ...
        for i in range(self.args.num_users):
            ...
            if(self.args.data_distribution == 101):
                for k in range(0,10):
                    for j in distribution_data[k]:   #0 -> i, 每个client随机  , i->0 ,改固定, 加上users_list[i]每个分配
            ...
            if(self.args.data_distribution == 102):
                for k in range(0,10):
                    for j in distribution_data[10-k]:  #0 -> i, 每个client随机
                        for k in np.random.randint(0,len(labels_list_train[ad])-1, int(j/5)):
                            dic_train[i] = np.insert(dic_train[i], 0, labels_list_train[ad][k])
                        ad += 1
                    y = np.argsort(dic_train[i])
                    dic_train_copy[i] = dic_train[i][y]
